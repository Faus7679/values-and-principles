...
```mermaid
flowchart TD
    A[Opaque or exploitative AI] --> B[User harm]
    A --> C[Reduced trust]
    B --> D[Legal complaints]
    C --> E[Customer exit]
    D --> F[Higher costs]
    E --> F
    F --> G[Weaker long-term market performance]
```
...
```mermaid
flowchart LR
    A[Law and standards] --> B[Design requirements]
    B --> C[Testing and documentation]
    C --> D[Safer deployment]
    D --> E[Public trust]
```
...
```mermaid
flowchart LR
    A[Christian conviction] --> B[Respect human dignity]
    A --> C[Demand transparency]
    B --> D[Engage developers]
    C --> E[Engage policymakers]
    D --> F[Promote ethical AI]
    E --> F
```
